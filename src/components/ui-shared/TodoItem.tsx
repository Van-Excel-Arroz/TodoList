	...
	return (
		<>
			{view === 'grid' ? (
				<div
					key={todo.id}
					className={`grid grid-cols-12 border bg-white ring-offset-2 ring-slate-400 hover:ring-1 cursor-pointer relative py-[2px] m-2 px-2 rounded-lg select-none drop-shadow-sm transition-all duration-100`}
					style={{
				...
				<div
					key={todo.id}
					className={`flex flex-col border mx-2 cursor-pointer relative py-1 m-2 px-3 bg-white rounded-lg select-none drop-shadow-sm transition-all duration-100 ring-slate-400 ring-offset-2	 hover:ring-1`}
					style={{
						borderColor: isSelected ? `${accentColor}80` : '',
					}}
					onClick={handleTodoClick}
